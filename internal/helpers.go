...
func BigImgDesc(ss *static.Slide, imgPath string, yScale, xScale float64, caption, desc string) {
	img := show.ImageCaptionSize(imgPath, .35, .53, xScale, yScale, Libel28, caption)
	descs := strings.Split(desc, "\n")
	for i := 1; i < len(descs); i++ {
		// remove whitespace
		descs[i] = strings.TrimSpace(descs[i])
	}
	txt := show.TxtSetFrom(Gnuolane44, .65, .30, 0, .06, descs...)
	// todo: balance positions, sizes
	ss.Append(append(txt, img)...)
}
