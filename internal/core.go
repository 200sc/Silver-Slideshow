...
func addCore(i int, sslides []*static.Slide) {
	...
	ImgDesc(sslides[i+1], "transformation_diagram_7.png", .55, .55,
	...
	DoubleImage(sslides[i+4],
		"no_code_gen_trans.png", .6, .5, "Code to apply 'simp' and 'expd' without generating code",
		"code_gen_trans.png", .25, .4, "What we provide to generate 'simp' and 'expd'",
	)
	ImgDesc(sslides[i+5], "restore_rules.png", .45, .45,
	...
	ImgDesc(sslides[i+6], "transformation_diagram_5.png", .55, .55,
